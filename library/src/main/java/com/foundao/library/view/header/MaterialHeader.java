package com.foundao.library.view.header;
...
public class MaterialHeader extends ViewGroup implements RefreshHeader {
    ...
    private CircleRefreshImageView mCircleView;
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mCircleView = new CircleRefreshImageView(context,CIRCLE_BG_LIGHT);
...
//                resolveSize(getSuggestedMinimumHeight(), heightMeasureSpec));
    ...
    public void onReleasing(float percent, int offset, int height, int extendHeight) {
        if (!mProgress.isRunning() && !mFinished) {
            onPullingDown(percent, offset, height, extendHeight);
        } else {
            if (mShowBezierWave) {
                mHeadHeight = Math.min(offset, height);
                mWaveHeight = Math.max(0, offset - height);
    ...
    @Override
    public void onRefreshReleased(RefreshLayout layout, int height, int extendHeight) {
        mProgress.start();
